<script lang="ts">
    ...
    async function initEditor() {
        ...
        monaco.editor.defineTheme('ddp-theme-dark', {
            ...
            inherit: true,
            colors: {},
            rules: ddpThemeData.dark as MonacoEditor.editor.ITokenThemeRule[]
        });
        ...
        editor = monaco.editor.create(editorDiv!, {
            ...
            model: model,
            minimap: { enabled: !settings.embedded },
            readOnly: settings.readOnly,
            lineNumbers: settings.nolines ? "off" : "on",
        ...
        monaco.languages.setLanguageConfiguration('ddp', ddpLanguageConfig as MonacoEditor.languages.LanguageConfiguration)
        ...
        ls_socket.onmessage = (event) => {
            ...
            switch (msg.method) {
                case 'textDocument/publishDiagnostics':
                    ...
                    if (model) monaco.editor.setModelMarkers(model, 'ddp', markers);
                    break;
                default:
        ...
        function handleInitializeResponse(resp: any) {
            ...
            monaco.languages.registerDocumentSemanticTokensProvider('ddp', {
                ...
                provideDocumentSemanticTokens: async (model, lastResultId, token) => {
                    ...
                    return push_response_handler().then((resp: any) => {
                        if (!resp.result) {
                            return null;
                        }
        ...
        if (!settings.readOnly) {
            ...
            editor!.onDidChangeModelContent((event) => {
                ...
                cached_changes.changes.push(...event.changes as MonacoEditor.editor.IModelContentChange[]);
        ...
        function doChangeRequest(changes: MonacoEditor.editor.IModelContentChange[]) {
            send({
                ...
                params: {
                    ...
                    contentChanges:
                        ...
                        changes.map((change: MonacoEditor.editor.IModelContentChange) => {
                            return {
                                range: {
                                    start: {
...
<div id="editor-container">
    ...
    {:catch error}
        <div class="error-overlay">
            <p>Fehler beim Laden des Editors: {error.message}</p>
        </div>
    {/await}
</div>
...
<style>
    ...
    .error-overlay {
        ...
        background: var(--controls-background-color, #1e1e1e);
    }
